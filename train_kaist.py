...
from keras_csp import config, data_generatorsrgbt
...
cache_ped = 'data/cache/KAIST/train_gt'
cache_emp = 'data/cache/KAIST/train_nogt'
...
data_gen_train = data_generatorsrgbt.get_data_hybrid(ped_data, emp_data, C, batchsize=batchsize, hyratio=0.5)
...
if C.offset:
    out_path = 'output/valmodels/KAIST/%s/off2' % (C.scale)
else:
    out_path = 'output/valmodels/KAIST/%s/nooff' % (C.scale)
...
for epoch_num in range(C.num_epochs):
    ...
    while True:
        try:
            ...
            iter_num += 1
            if iter_num % 20 == 0:
                progbar.update(iter_num,[('cls', np.mean(losses[:iter_num, 0])), ('regr_h', np.mean(losses[:iter_num, 1])), ('offset', np.mean(losses[:iter_num, 2]))])
