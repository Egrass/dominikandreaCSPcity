...
def nn_p3p4(img_input=None, offset=True, num_scale=1, trainable=False):
...
def conv_block(x, stage, branch, nb_filter, trainable, dropout_rate=None, weight_decay=1e-4):
    ...
    x = Convolution2D(nb_filter, (3, 3),  name=conv_name_base+'_x2', use_bias=False, trainable=trainable)(x)
...
def dense_block(x, stage, nb_layers, nb_filter, growth_rate, trainable, dropout_rate=None, weight_decay=1e-4, grow_nb_filters=True):
    ...
    for i in range(nb_layers):
        branch = i+1
        x = conv_block(concat_feat, stage, branch, growth_rate, trainable,dropout_rate, weight_decay)
