...
data_gen_train = data_generators.get_data_hybridms4anchorcls(train_data, C, batchsize=batchsize)
...
if C.network == 'resnet50':
    ...
    # weight_path = '/home/xwj/CSPoutput/valmodels/city/h/off2cam/net_e46_l0.0117170121482.hdf5'
    weight_path = '/home/xwj/CSPoutput/valmodels/city/h/off2camms1/net_e150_l0.0114874236278.hdf5'
    #weight_path = '/media/xwj/Data/models_CSP/cityperson/net_e121_l0(3).hdf5'
if C.offset:
    out_path = '/home/xwj/CSPoutput/valmodels/city/%s/off2camms11' % (C.scale)
...
preds = nn.nn_p3p4p5_ms4anchorcls(img_input, offset=C.offset, num_scale=C.num_scale, trainable=True)
preds_tea = nn.nn_p3p4p5_ms4anchorcls(img_input, offset=C.offset, num_scale=C.num_scale, trainable=True)
...
if C.offset:
    model.compile(optimizer=optimizer,loss=[losses.cls_center008,losses.cls_centerms4, losses.regr_hms4, losses.regr_offset])
...
total_loss_r,cls_loss_r,  cls_loss_r1, regr_loss_r1, offset_loss_r1 = [], [], [], [], []
for epoch_num in range(C.num_epochs):
    ...
    while True:
        try:
            ...
            losses[iter_num, 3] = loss_s1[4]
            iter_num += 1
            if iter_num % 20 == 0:
                progbar.update(iter_num,
                               [('cls', np.mean(losses[:iter_num, 0])),('clsr', np.mean(losses[:iter_num, 1])),
                                ('regr_h', np.mean(losses[:iter_num, 2])) ,('offset', np.mean(losses[:iter_num, 3]))])
            if iter_num == epoch_length:
                cls_loss = np.mean(losses[:, 0])
                cls_loss1 = np.mean(losses[:, 1])
                regr_loss1 = np.mean(losses[:, 2])
                offset_loss1 = np.mean(losses[:, 3])


                total_loss = cls_loss+ cls_loss1 + regr_loss1+offset_loss1


                total_loss_r.append(total_loss)
                cls_loss_r.append(cls_loss)
        ...
        records = np.concatenate((np.asarray(total_loss_r).reshape((-1, 1)),
                                  np.asarray(cls_loss_r).reshape((-1, 1)),
