...
#modelcsp.load_weights('/home/xwj/CSPoutput/valmodels/city/h/off2camcls1/net_e23_l0.269329622753.hdf5', by_name=True)


if C.offset:
    w_path = '/home/xwj/CSPoutput/valmodels/city/%s/off2camms1/' % (C.scale)
    out_path = '/home/xwj/CSPoutput/valresults/city/%s/off2camms111' % (C.scale)

if not os.path.exists(out_path):
    os.makedirs(out_path)
files = sorted(os.listdir(w_path))
# get the results from epoch 51 to epoch 150
for w_ind in range(120, 124,1):
    for f in files:
    ...
    for f in range(num_imgs):
        ...
        if C.offset:
            ...
            boxes = bbox_process.parse_det_offset_ms4maxcls(Ycsp,Y, C, score=0.01, down=4)
        else:
