...
data_gen_train = data_generators.get_data_cls(train_data, C, batchsize=batchsize)
...
if C.network == 'resnet50':
    ...
    weight_path = '/home/xwj/CSPoutput/valmodels/city/h/off2camcls/net_e30_l0.00335340754119.hdf5'
    weight_path = '/home/xwj/CSPoutput/valmodels/city/h/off2camcls1/net_e39_l0.279456728878.hdf5'
if C.offset:
    out_path = '/home/xwj/CSPoutput/valmodels/city/%s/off2camcls1024' % (C.scale)
...
preds = nn.nn_p3p4p5cancls(img_input, offset=C.offset, num_scale=C.num_scale, trainable=True)
preds_tea = nn.nn_p3p4p5cancls(img_input, offset=C.offset, num_scale=C.num_scale, trainable=True)
...
if C.offset:
    model.compile(optimizer=optimizer, loss=[losses.acls_center])
...
for epoch_num in range(epoch, C.num_epochs):
    ...
    while True:
        ...
        # print loss_s1
        losses[iter_num, 0] = loss_s1


        iter_num += 1
        if iter_num % 20 == 0:
            progbar.update(iter_num,[('cls', np.mean(losses[:iter_num, 0]))])
        if iter_num == epoch_length:
            cls_loss1 = np.mean(losses[:, 0])

            total_loss = cls_loss1
